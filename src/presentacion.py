...
def get_ubicacion_vial(descripcion):
    location = ['calle', r'garaje', r'roton\w*', 'autopista', 'avenida', 'cruce', 'cruze', r'esquina\w*',
                r'estacionami\w*', 'carril', 'ruta', r'semaforo\w*', 'semáforo\w*', r'intersec.?', 'tunel', 'peaje', 'garage']
    aux = []
    for loc in location:
...
if len(input_text):
    desc_vec = vectorizer1.transform(
        [' '.join([sbs.stem(word) for word in input_text.split()])])
    resp_pred = svm_responsabilidad.predict(desc_vec)[0]
    resp_pred = 'responasble' if bool(resp_pred) else 'no responsable'
    # resp_pred
...
if st.button('Caso aleatorio'):
    ...
    # resp_base = 'responsable' if muestra['responsabilidad'].bool else 'no responsable'
    ...
    resp_pred = svm_responsabilidad.predict(desc_vec)[0]
    resp_pred = 'responsable' if bool(resp_pred) else 'no responsable'
    # resp_pred
    st.text('Responsabilidad predicha: ' + resp_pred)
    ...
    lugar_pred = ml.predict(desc_vec)[0]
    st.text('El daño en el vehículo asegurado se encuentra en la parte: ' +
            dot_prod(lugar, lugar_pred) if 1 in lugar_pred else 'El daño en el vehículo asegurado se encuentra en la parte: no identificado')
